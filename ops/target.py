...
def get_fpn_location_coords(
    ...
    for level_name, feat_shape in shape_per_fpn_level.items():
        ...
        # Unpack feature shape (B, C, H, W)
        _, _, h, w = feat_shape

        # Generate grid of coordinates
        x_coords = torch.arange(0, w * level_stride, step=level_stride, dtype=dtype, device=device) + level_stride / 2
        y_coords = torch.arange(0, h * level_stride, step=level_stride, dtype=dtype, device=device) + level_stride / 2
